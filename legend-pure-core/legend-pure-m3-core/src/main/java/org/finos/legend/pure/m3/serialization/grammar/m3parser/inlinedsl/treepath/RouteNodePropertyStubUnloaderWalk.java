...
public class RouteNodePropertyStubUnloaderWalk implements MatchRunner<RouteNodePropertyStub>
{
    @Override
    public String getClassName()
    {
        return M3Paths.RouteNodePropertyStub;
    }

    @Override
    public void run(RouteNodePropertyStub propertyRouteNode, MatcherState state, Matcher matcher, ModelRepository modelRepository, Context context) throws PureCompilationException
    {
        RouteNode ownerNode = propertyRouteNode._owner();
        if (ownerNode instanceof RootRouteNode)
        {
            matcher.fullMatch(ownerNode, state);
        }
        else
        {
